...
class Solution:
    def getFolderNames(self, names: List[str]) -> List[str]:
        idxMap = dict() # 表示当前名字的(k)从哪里开始
        res = []

        for name in names:
            if name not in idxMap:
                # 名字不在 map 中直接添加进来，然后map值置1
                idxMap[name] = 1
                res.append(name)
            else:
                # 如果新名字在 map 中，就去除对应的 idx 然后重新组成新名字（新名字中必须是原来的 name 加上 (k)）
                newName = name
                while newName in idxMap:
                    idx = idxMap[name]
