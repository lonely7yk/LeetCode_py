...
class Solution:
    def cherryPickup(self, grid: List[List[int]]) -> int:
        ...
        def dfs(row, col1, col2):
            ...
            if row != m - 1:            
                res += max(dfs(row + 1, nc1, nc2) for nc1 in [col1, col1- 1, col1 + 1] for nc2 in [col2, col2 - 1, col2 + 1])
...
#         dp = [[[0 for k in range(n)] for j in range(n)] for i in range(m)]
...
#                     tmp = 0
#                     for nc1 in [j, j - 1, j + 1]:
#                         for nc2 in [k, k - 1, k + 1]:
#                             if 0 <= nc1 < n and 0 <= nc2 < n:
#                                 tmp = max(tmp, dp[i + 1][nc1][nc2])
