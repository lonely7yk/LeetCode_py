...
class Solution:
    ...
    def getOrder(self, tasks: List[List[int]]) -> List[int]:
        sorted_tasks = sorted([(t[0], t[1], i) for i, t in enumerate(tasks)], reverse=True)
        heap = []
        res = []

        end_time = sorted_tasks[-1][0]

        while sorted_tasks or heap:
            while sorted_tasks and sorted_tasks[-1][0] <= end_time:
                sorted_task = sorted_tasks.pop()
                heapq.heappush(heap, (sorted_task[1], sorted_task[2], sorted_task[0]))

            if heap:
                heap_task = heapq.heappop(heap)
                end_time += heap_task[0]
                res.append(heap_task[1])
            else:
                end_time = sorted_tasks[-1][0]
