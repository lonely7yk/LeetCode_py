...
consecutive elements is the same. More formally, a sequence s is arithmetic if and only if s[i+1] - s[i] == s[1] - s[0]
for all valid i.
...
class Solution:
    def checkArithmeticSubarrays(self, nums: List[int], l: List[int], r: List[int]) -> List[bool]:
        def check(A):
            mn, mx, st = min(A), max(A), set(A)
            if (mx - mn) % (len(A) - 1) != 0: return False
            step = (mx - mn) // (len(A) - 1)
