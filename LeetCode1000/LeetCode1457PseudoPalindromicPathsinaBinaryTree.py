...
class Solution:
    def pseudoPalindromicPaths (self, root: TreeNode) -> int:
        # 如果结点存在 set 中，就删除节点，否则添加节点
        def setNode(oddNodeSet, node):
            if node.val in oddNodeSet: oddNodeSet.remove(node.val)
            else: oddNodeSet.add(node.val)
        
        def dfs(node, oddNodeSet):
            setNode(oddNodeSet, node)
            
            # 叶子节点判断奇数个数的节点个数是否小于 1
            ...
            res = 0
            if node.left:
                res += dfs(node.left, oddNodeSet)
            if node.right:
                res += dfs(node.right, oddNodeSet)
