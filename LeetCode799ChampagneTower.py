...
on until the 100th row.  Each glass holds one cup of champagne.
...
# # Simulation: O(n^2) - O(n^2)
# class Solution:
#     def champagneTower(self, poured: int, query_row: int, query_glass: int) -> float:
#         # A[i][j] 表示有多少流过
#         A = [[0 for j in range(i)] for i in range(1, query_row + 2)]
#         A[0][0] = poured
        
#         for i in range(query_row):
#             for j in range(i + 1):
#                 excess = max(A[i][j] - 1, 0)
#                 A[i + 1][j] += excess / 2
#                 A[i + 1][j + 1] += excess / 2
                
#         return min(A[query_row][query_glass], 1)

# Improve: O(n^2) - O(n)
class Solution:
    def champagneTower(self, poured: int, query_row: int, query_glass: int) -> float:
        def flow(A1, A2):
            for i in range(len(A2)): A2[i] = 0
        ...
        A1 = [0 for i in range(query_row + 1)]
        A2 = [0 for i in range(query_row + 1)]
        ...
        if not flag: return min(1, A2[query_glass])
        else: return min(1, A1[query_glass])
