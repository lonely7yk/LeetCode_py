...
#         def checkWord(trie, word, cnt):
#             if not word: return cnt >= 2
#
#             p = trie.root
#             for i, c in enumerate(word):
#                 if c not in p.children: return False
#                 p = p.children[c]
#                 if p.isWord and checkWord(trie, word[i + 1:], cnt + 1):
#                     return True
#
#             return False
#
...
# # DP: O(n^3)
# # 参考：LC139 word break
# # reference: https://leetcode.com/problems/concatenated-words/discuss/95652/Java-DP-Solution
# class Solution:
#     def findAllConcatenatedWordsInADict(self, words: List[str]) -> List[str]:
#         # DP，这个和 word break 方法是一样的
...
class Solution:
    def findAllConcatenatedWordsInADict(self, words: List[str]) -> List[str]:
        ...
        # 判断单词是否能使用 wordSet 中的单词组成
        def dfs(word):
            if word in memo: return memo[word]

            # 这里 i 的范围是 [1,len(word)-1]，以便让 prefix 和 suffix 都至少有一个字符
